...
//fetchAllPostsByTag("Software Developer");
searchPostings();
 // create user 
...
{
  var ref = firebase.database().ref("jobs/postings");
var newArray = [];
...
function fetchAllPostsByTag(tags)
{
  var jobsRef = firebase.database().ref("jobs/postings/");
  var newArray = [];
  jobsRef.child("tags".includes(tags)).on("child_added", function(snapshot) {
    newArray.push(snapshot.val);
...
jobsRef.on("value", function(snapshot) {  
  ...
  {
   // console.log(key);
   // console.log(allvalues[key].company_address);
    
   } 
    filterData(allvalues);
}, function (errorObject) {
  console.log("The read failed: " + errorObject.code);
});
}

function filterData(allvalues)
{
var _ = require("underscore");
var filtered = _.where(allvalues, { job_type:'FullTime'});
//var keys = _.keys(allvalues);
//console.log(keys);
...
for(var key in filtered)
  {
    ...
    console.log(filtered[key].company);
    
  } 

//   var filteredIds = _.filter(_.keys(allvalues), function (key) {
...
function searchEmployers(searchOn, searchByVal)
{
  ...
  // replace location by searchOn and the value by searchByVal 
jobsRef.orderByChild("city").equalTo('Markham Road').on("child_added", function(snapshot) {
